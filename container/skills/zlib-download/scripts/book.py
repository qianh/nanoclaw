...
# Use /workspace/extra/books if running inside NanoClaw container, else ~/Downloads
_NANOCLAW_BOOKS_DIR = Path("/workspace/extra/books")
DEFAULT_DOWNLOAD_DIR = _NANOCLAW_BOOKS_DIR if _NANOCLAW_BOOKS_DIR.exists() else Path.home() / "Downloads"
...
def _send_file_ipc(file_url: str, filename: str, file_type: int = 4, local_path: str = ""):
    ...
    if not chat_jid:
        return  # Not running inside NanoClaw, skip
...
def annas_download(args):
    ...
    # Automatically send file to chat via IPC (if running inside NanoClaw)
    # Anna's Archive doesn't give a CDN URL, so skip IPC send for now
